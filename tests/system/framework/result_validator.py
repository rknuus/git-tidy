...
class RepositoryState:
    ...
    def __init__(self, repo_path: Path):
        ...
        self.commit_count = 0 if self.is_empty else len(list(self.repo.walk(self.repo.head.target)))

        # Capture branches
        self.branches = set(self.repo.branches.local)

        # Capture tags
        self.tags = {ref.split("/")[-1] for ref in self.repo.references if ref.startswith("refs/tags/")}
            ...
            for file_path in self.repo_path.rglob("*"):
                if file_path.is_file() and ".git" not in file_path.parts:
                    try:
                        ...
                        checksums[str(file_path.relative_to(self.repo_path))] = hashlib.sha256(
                            content
                        ).hexdigest()
                    except Exception:
...
class ResultValidator:
    ...
    def _validate_success_with_changes(
        self, result: GitTidyResult, pre_state: RepositoryState, post_state: RepositoryState
    ) -> None:
        """Validate successful execution with repository changes."""
        if result.exit_code != 0:
            raise ValidationError(f"Expected success but got exit code {result.exit_code}: {result.stderr}")

        # Check that repository state changed
        if pre_state.head_sha == post_state.head_sha and pre_state.commit_count == post_state.commit_count:
            raise ValidationError("Expected changes but repository state appears unchanged")
    ...
    def _validate_conflict_reported(
        ...
        conflict_indicators = ["conflict", "merge conflict", "unable to merge", "conflicting changes"]

        output_text = (result.stdout + result.stderr).lower()
        if not any(indicator in output_text for indicator in conflict_indicators):
            raise ValidationError("Expected conflict reporting but none found in output")
    ...
    def _validate_repository_integrity(self, post_state: RepositoryState) -> None:
        ...
        try:
            ...
            if result.returncode != 0 and "error:" in result.stderr.lower():
                raise ValidationError(f"Repository integrity check failed: {result.stderr}")

        except Exception as e:
            raise ValidationError(f"Failed to run repository integrity check: {e}")
    ...
    def _validate_content_preservation(
        ...
        preserved_files = set(pre_state.file_checksums.keys()) & set(post_state.file_checksums.keys())
    ...
    def validate_backup_created(self, repo_path: Path, expected: bool = True) -> None:
        ...
        backup_branches = [branch for branch in repo.branches.local if branch.startswith("backup-")]
        ...
        if expected and not backup_branches:
            ...
            raise ValidationError(f"Unexpected backup branch created: {backup_branches}")

    def validate_commit_count_change(
        self, pre_state: RepositoryState, post_state: RepositoryState, expected_change: int
    ) -> None:
