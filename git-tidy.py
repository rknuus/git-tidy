...
class GitTidy:
    ...
    def create_rebase_todo(self, groups: List[List[Dict]]) -> str:
        ...
        for group_idx, group in enumerate(groups):
            ...
            for commit_idx, commit in enumerate(group):
                action = "pick"
    ...
    def describe_group(self, group: List[Dict]) -> str:
        ...
        if len(all_files) <= 3:
            ...
            sample_files = sorted(list(all_files))[:3]
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Tidy up git commits. Currently groups git commits by file similarity.')
